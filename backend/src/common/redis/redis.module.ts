  ...
  providers: [
    {
      ...
      useFactory: async (configService: ConfigService): Promise<Redis> => {
        ...
        // Normalize boolean values from config service
        ...
        // Prevent disabling TLS certificate verification in production
        if (isProduction && tlsEnabled && !tlsRejectUnauthorized) {
          throw new Error('Redis TLS certificate verification cannot be disabled in production');
        }

        // Normalize CA certificate: replace literal \n with actual newlines
        const tlsCaRaw = configService.get<string>('redis.tlsCa');
        const tlsCa = tlsCaRaw?.replace(/\\n/g, '\n');
        ...
        const redis = new Redis({
          ...
          db: configService.get<number>('redis.db', isTest ? 1 : 0),
          ...
          commandTimeout: isTest ? 3000 : 5000,

          tls: tlsEnabled
            ? {
                servername: configService.get<string>('redis.tlsServername') || undefined,
        ...
        redis.on('connect', () => {
          logger.debug('Redis connection established (socket connected)');
        });
        ...
        redis.on('reconnecting', (delay: number) => {
          reconnectCount++;
          // Log warn only every 5th reconnection attempt to reduce noise
          if (reconnectCount % 5 === 0 || reconnectCount === 1) {
            logger.warn(`Redis reconnecting in ${delay}ms (attempt ${reconnectCount})`);
          } else {
            logger.debug(`Redis reconnecting in ${delay}ms (attempt ${reconnectCount})`);
          }
        });
