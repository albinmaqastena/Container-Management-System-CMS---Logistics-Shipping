...
describe('ItemsService', () => {
  ...
  describe('restore', () => {
    it('should restore an item and return it', async () => {
      ...
      itemRepo.findOne.mockResolvedValueOnce(deletedItem).mockResolvedValueOnce(restoredItem);
      itemRepo.restore.mockResolvedValue({ affected: 1 });
      itemRepo.createQueryBuilder.mockReturnValue(createSumQueryBuilder('12.5'));
      containerRepo.update.mockResolvedValue({ affected: 1 });
      containersService.findOneIncludingDeleted.mockResolvedValue(mockContainer);
      dataSource.transaction.mockImplementation(async (callback) => callback(manager));
      ...
      expect(containersService.findOneIncludingDeleted).toHaveBeenCalledWith('container-1');
    });
