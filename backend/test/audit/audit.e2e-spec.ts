...
describe('Audit E2E', () => {
  ...
  describe('GET /v1/audit', () => {
    ...
    it('should filter by a valid user UUID', async () => {
      ...
      const response = await request(
        ...
        .get(
          `/v1/audit?userId=${logWithUserId.userId}`,
        )
        .set(
          'Authorization',
          `Bearer ${superAdminToken}`,
    ...
    it('should filter by fromDate and toDate', async () => {
      ...
      for (const log of response.body.data) {
        ...
        expect(createdAt).toBeLessThanOrEqual(
          toDate.getTime(),
        );
      }
    });
    ...
    it('should return 400 for invalid fromDate', async () => {
      await request(app.getHttpServer())
        .get(
          '/v1/audit?fromDate=invalid-date',
        )
        .set(
          'Authorization',
          `Bearer ${superAdminToken}`,
  ...
  describe('GET /v1/audit/users/:userId', () => {
    it('should return audit logs for an existing user ID', async () => {
      ...
      const response = await request(
        ...
        .get(
          `/v1/audit/users/${logWithUserId.userId}`,
        )
        .set(
          'Authorization',
          `Bearer ${superAdminToken}`,
    ...
    it('should return an empty page for a valid unknown user UUID', async () => {
      const response = await request(
        ...
        .get(
          `/v1/audit/users/${nonExistentUuid}`,
        )
        .set(
          'Authorization',
          `Bearer ${superAdminToken}`,
  ...
  describe('DELETE /v1/audit/cleanup', () => {
    const destructiveIt =
      process.env.RUN_DESTRUCTIVE_E2E ===
      'true'
        ? it
        : it.skip;

    destructiveIt(
      'should clean up old audit logs with default retention',
      async () => {
        const response = await request(
          app.getHttpServer(),
        )
    ...
    destructiveIt(
      'should clean up old audit logs with custom retention',
      async () => {
        const response = await request(
          app.getHttpServer(),
        )
          .delete(
            '/v1/audit/cleanup?days=30',
          )
          .set(
            'Authorization',
            `Bearer ${superAdminToken}`,
